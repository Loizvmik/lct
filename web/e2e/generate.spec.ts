...
test("полный путь: шаблон, задание, варианты, проверка и скачивание", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Готовим три варианта" })).toBeVisible();
  await expect(page.locator(".stage-track .stage")).toHaveCount(6);

  await page.waitForURL(/\/decks\/[a-f0-9]+\/variants$/, { timeout: 5 * 60 * 1000 });
  await expect(page.getByRole("heading", { name: "Выберите подачу" })).toBeVisible();
...
test("пустая форма, черновик и проверка числа слайдов", async ({ page }) => {
  ...
  await page.evaluate((id) => {
    localStorage.removeItem(`slides:task-draft:v2:${id}`);
    localStorage.setItem(`deckforge:brief-draft:${id}`, JSON.stringify({
      title: "Сокращение времени согласования заявок",
      brief: "Запуск автоматической маршрутизации заявок",
