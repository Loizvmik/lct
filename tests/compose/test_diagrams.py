...
# чисел и метода замера). Три разных ответа на трёх шаблонах — то самое
# различение, которого не было у старой (переписной) версии словаря.
_EXPECTED_DOMINANT_CARD_SHAPE = {
    "VK Tech шаблон.pptx": "rect",
    "VK_WorkSpace_Клиентская_конференция_Шаблон_03.pptx": "ellipse",
    "Шаблон презентации VK Education.pptx": "ellipse",
}
...
def test_diagram_card_uses_the_templates_dominant_shape(new_slide, profile_fixture, BOX, template_name):
    """Task 10 код-ревью, находка №1 (критично): словарь форм раньше
    считался ПЕРЕПИСЬЮ всех автофигур пакета (слайды+макеты+мастера) —
    прямоугольники макетов перевешивали язык карточек, и `rect` побеждал
    на всех трёх учебных шаблонах (см. докстроку `template/shapes.py` про
    живой замер). Тест раньше проверял только, что выбранная форма
    ВСТРЕЧАЕТСЯ в словаре шаблона — `rect` встречается везде, тест проходил
    всегда и различения не ловил. Теперь — что схема использует ИМЕННО
    ПРЕОБЛАДАЮЩУЮ форму словаря (`profile.shape_vocabulary[0]`), и что эта
    преобладающая форма у разных шаблонов РАЗНАЯ."""
...
def test_all_three_templates_do_not_all_agree_on_one_shape(profile_fixture):
    ...
    тест выше всё ещё проходит на СВОИХ ожидаемых значениях."""
    winners = {
