...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    for variant in variants:
        ...
        # Задача T: сводка верности шаблону на вариант — не входит в бюджет
        # прогона (печать, не решение пайплайна) и не должна ронять
        # генерацию, если что-то в метрике пошло не так (профиль в кэше
        # старше кода метрики и т.п.) — та же честная деградация, что и у
        # остальных необязательных шагов этого файла.
        try:
            fidelity_report = template_fidelity(path, variant_deck, profile)
            print(f"  {fidelity_report.summary}")
        except Exception as exc:  # noqa: BLE001 — сводка необязательна, генерация не должна упасть из-за неё
            print(f"  ! Верность шаблону не посчитана: {exc}")
