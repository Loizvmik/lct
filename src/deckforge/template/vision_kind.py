...
# три учебных файла.
#
# 0.5 -> 0.8 (25 сентября 2026). Порог «ниже монетки» верен для вопроса
# «доверять ли источнику», но здесь вопрос другой: «стоит ли переспросить».
# Цена вопроса — один вызов модели на раскладку в РАЗБОРЕ шаблона, а он
# делается один раз на файл, кешируется и в пятиминутный бюджет генерации
# не входит вовсе (уточнение заказчика от 23 сентября: подготовительный
# этап во времени не ограничен). Цена ошибки в другую сторону — раскладка
# на всю жизнь остаётся в корзине по умолчанию: `patterns._classify_kind`
# отдаёт `("bullets", 0.3)`, когда не узнала форму, и таких на VK Education
# было 22 из 33. При 0.5 модель не спрашивают про раскладки с уверенностью
# 0.5-0.8 — а это как раз «геометрия что-то предполагает, но неточно».
_ASK_CONFIDENCE_THRESHOLD = 0.8
