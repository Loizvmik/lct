...
# Короткий ответ (один JSON-объект `{"kind": "..."}`) — тот же порядок, что
# `naming.py`/`pattern-picker` (не многосоставной JSON аудита по картинке,
# которому нужен куда больший бюджет, см. `audit.visual` про перекос
# бюджета в reasoning_content у qwen3.6).
_MAX_TOKENS = 512
