...
# 4074, 4298, 4569, 4894, 4993, 5116, 5150, 5181, 5237, 5779, 6180 (11 замеров,
# см. предыдущую находку выше).
#
# "Разбор незнакомого шаблона в бюджет", продолжение 3 — тот же класс находки,
# что уже дважды чинился в проекте (здесь и у `vision_kind._TOKENS_PER_ITEM`):
# именование палитры отдельно от остального разбора стоило 60.8с САМИХ ПО
# СЕБЕ на контрольном ЛЦТ2026 (см. task-18-report.md, "Продолжение 2") — почти
# весь дедлайн роли (60с), потому что на этом конкретном шаблоне 5632 не
# покрывает типичный ответ, и вызов регулярно уходит в эскалацию бюджета
# (лишний сетевой круг, десятки секунд). Перемерено ЖИВЬЁМ ещё раз, теперь
# ЦЕЛЕНАПРАВЛЕННО на контрольном шаблоне (6 прогонов `llm.complete(...)` на
# ТЕХ ЖЕ входных данных, что строит `_ask_model` ниже, `max_tokens=16000`,
# чтобы увидеть настоящую стоимость ответа, а не `finish_reason=length` от
# заниженного потолка — тот же приём калибровки, что и у `vision_kind.py`):
# 4683, 4782, 5324, 5603, 6304, 6406 — ВСЕ шесть finish_reason=stop, ни одного
# сбоя. Вместе с прежними 11 замерами (три учебных шаблона) — 17 живых точек
# данных:
#
#   4074, 4298, 4569, 4683, 4782, 4894, 4993, 5116, 5150, 5181, 5237, 5324,
#   5603, 5779, 6180, 6304, 6406
#
# 5632 (было) покрывал без эскалации только 13 из 17 — именно контрольный
# ЛЦТ2026 (новые 6 замеров) даёт самый широкий разброс во всей выборке,
# включая ДВА ответа (6304, 6406) выше потолка эскалации 6144. Поднято до
# 6100: покрывает БЕЗ единой эскалации 14 из 17 (добавляет 5779 — раньше
# уходивший в лишний сетевой круг), при этом остаётся ниже потолка 6144 с
# запасом, достаточным, чтобы escalation (next = min(current*2, 6144)) всё
# ещё делала один реальный шаг для оставшихся случаев (любое стартовое
# значение из диапазона (3072, 6144) удваивается ВЫШЕ потолка и обрезается
# до него за один шаг — величина запаса ниже потолка не влияет на то,
# срабатывает ли эскалация, только на то, СКОЛЬКО вызовов вообще успевают до
# неё дойти; чем выше старт в этом диапазоне, тем меньше вызовов тратят
# лишний сетевой круг на предсказуемо успешный ответ).
#
# Честная оговорка (не снята, а усилена этим замером): потолок эскалации
# (6144) не гарантированно достаточен для этого вызова — раньше это был один
# живой прогон (6180) из 11, теперь ДВА из 17 (6304, 6406), оба именно на
# контрольном шаблоне. Эскалация для них сделает шаг до потолка и всё равно
# не дотянется — обрезанный ответ, не пригодный для `json.loads`, и роли
# уйдут в запасной вариант с честной заметкой (см. `_ask_model` ниже). Это
# риск потолка провайдера (`yandex.MAX_TOKENS_BUDGET_CAP`), не устраняемый
# одним лишь стартовым бюджетом, и — тем же выбором, что уже сделан
# `vision_kind.py` для своего провайдерского потолка — вне периметра этой
# правки (naming.py/profile.py/config/app.yaml, не yandex.py).
_PALETTE_NAMING_INITIAL_MAX_TOKENS = 6100
