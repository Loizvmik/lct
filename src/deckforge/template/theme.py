...
def pick_primary_master(pkg: PptxPackage) -> str:
    ...
    Ранжирование, по убыванию приоритета:
    1. читаемость темы — мастер, чья тема физически не читается (нет
       relationship theme, битый Target, невалидный XML), отправляется в
       конец пула и участвует в отборе, только если читаемых кандидатов
       нет вовсе (см. ниже);
    2. среди читаемых — не стоковая тема (см. is_stock_office): мастеров
       со стоковой офисной палитрой, привязанной напрямую к мастеру, в
       разведанных файлах не встретилось, но пренебрегать этим нельзя —
       разведка это не гарантирует для незнакомого шаблона с защиты;
    3. среди оставшихся — число лейаутов, тай-брейк — число слайдов, чей
       лейаут в итоге ссылается на этот мастер.

    Task 3 повторное код-ревью, дожатие находки adversarial-reviewer: раньше
    читаемость вообще не входила в ранжирование — нечитаемая тема считалась
    просто "не стоковой" (см. докстроку _probe_master_theme) и наравне с
    читаемыми участвовала в тай-брейке по числу лейаутов. Мастер, у которого
    в файл приехали чужие слайды без связей (частая порча при копировании
    слайдов между презентациями), мог нести больше лейаутов, чем исправный
    рядом, и выигрывал отбор — а дальше `read_theme` на нём падал, и `read_theme`
    ронял весь разбор, хотя рядом был полностью годный кандидат. Теперь
    читаемость стоит выше числа лейаутов: нечитаемый мастер берётся, только
    если ЗАМЕНИТЬ его нечем (читаемых кандидатов нет вовсе) — тогда он
    по-прежнему может стать первичным и упасть на `read_theme()` с внятным
    сообщением, ровно как раньше ("первичный мастер — особый случай: если
    не читается он, падать правильно" — актуально теперь только когда не
    читается НИ ОДИН мастер).

    Читаемость и стоковость here считаются одним чтением на мастер, не
    двумя (см. _probe_master_theme) — темы мастеров и так читаются, второе
    чтение того же XML было бы лишним.
    """
    ...
    scored = [
        (master, *_probe_master_theme(pkg, master), len(pkg.related(master, "slideLayout")))
        for master in masters
    ]
    # row = (master, is_readable, is_stock, layout_count)
    readable = [row for row in scored if row[1]]
    ranked = readable if readable else scored
    non_stock = [row for row in ranked if not row[2]]
    pool = non_stock if non_stock else ranked

    max_layouts = max(row[3] for row in pool)
    tied = [row[0] for row in pool if row[3] == max_layouts]
    if len(tied) == 1:
        return tied[0]
...
def _probe_master_theme(pkg: PptxPackage, master_part: str) -> tuple[bool, bool]:
    """(is_readable, is_stock) — одна попытка чтения темы мастера, без
    повторного чтения того же XML (см. докстроку pick_primary_master):
    и отбор по читаемости, и отсев стоковой палитры используют один и тот
    же результат.

    is_readable=False означает, что тема физически не читается: нет
    relationship theme, часть без a:themeElements, Target указывает на
    файл, которого нет в архиве, либо файл есть, но не well-formed XML.
    Для такого мастера is_stock тоже False — не "стоковый" и не
    "неизвестно": тот же принцип асимметричной безопасности, что уже
    описан у is_stock_office (неполные/нерезолвленные данные всегда дают
    False, никогда не мешают отбору). Непрочитавшаяся тема ПЕРВИЧНОГО
    мастера, если он всё же будет выбран (читаемых кандидатов не было
    вовсе — см. pick_primary_master), проявится позже и честно, при прямом
    read_theme() в _ThemeGraph.
    ...
    try:
        ...
        return False, False
    return True, is_stock_office(scheme)
