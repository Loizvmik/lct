...
# ---------------------------------------------------------------------------
# CIE Lab / ΔE — попарная РАЗЛИЧИМОСТЬ цветов (Task 10 код-ревью, находка
# №2): `colorsys`-инструментарий выше (HLS) отвечает на вопрос "насколько
# насыщен/светел ОДИН цвет", не "отличит ли человек ДВА цвета рядом" — сырой
# RGB для второго вопроса не годится: одно и то же числовое расстояние между
# каналами выглядит по-разному в разных участках спектра (глаз намного
# чувствительнее к оттенкам зелёного, чем синего). CIE Lab строился именно
# так, чтобы евклидово расстояние в нём (ΔE) приближённо соответствовало
# ВОСПРИНИМАЕМОЙ разнице — см. `delta_e76`/`_srgb_to_lab` ниже, используется
# `template/chart_palette.py`, чтобы не пускать в одну палитру рядов графика
# два цвета, неотличимых на глаз (контрольный ЛЦТ2026 живьём даёт
# `#FE095F`/`#FF0053` — 15 из 441 в сыром RGB, ΔE76 у них при этом ≈ 7.0
# (собственный замер этой правки: `delta_e76("#FE095F", "#FF0053")`),
# заметно ниже порога уверенной различимости `chart_palette._MIN_DELTA_E`,
# см. `delta_e76`).
def _srgb_to_lab(hex_color: str) -> tuple[float, float, float]:
    """`"#RRGGBB"` → (L*, a*, b*) CIE 1976 Lab, иллюминант D65 — стандартная
    цепочка sRGB → линейный RGB (гамма-декодирование) → XYZ (матрица sRGB/
    D65, IEC 61966-2-1) → Lab (f(t), ECMA не при чём — это чистая колориметрия,
    не DrawingML)."""
    r, g, b = _hex_to_frgb(hex_color)
    ...
    # Матрица sRGB (D65) -> XYZ, IEC 61966-2-1.
    x = r_lin * 0.4124564 + g_lin * 0.3575761 + b_lin * 0.1804375
    y = r_lin * 0.2126729 + g_lin * 0.7151522 + b_lin * 0.0721750
    z = r_lin * 0.0193339 + g_lin * 0.1191920 + b_lin * 0.9503041

    # Опорный белый D65 (2° наблюдатель), нормировка X/Y/Z к нему.
...
def delta_e76(hex_a: str, hex_b: str) -> float:
    """ΔE CIE76 — евклидово расстояние в Lab между `hex_a` и `hex_b`.

    CIE76 — самая простая из формул ΔE (CIE94/CIEDE2000 взвешивают оси
    Lab неравномерно, точнее приближают восприятие в отдельных зонах
    спектра, но CIE76 уже даёт на порядок более честную оценку различимости,
    чем сырой RGB, и без лишней сложности калибровки весов — тот же
    практический компромисс "заметно лучше без переусложнения", что и у
    HLS-фильтра насыщенности этого же модуля."""
