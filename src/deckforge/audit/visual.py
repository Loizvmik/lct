...
def run_visual(
    ...
    *,
    ...
    пропускает коллаж C09/C11 (нужно только там, где связность колоды не
    интересует вызывающий код — задача M завела `deck_level=True` и в
    пайплайн, `workflow.visual_stage.run_visual_stage`, коллаж один раз на
    вариант поверх рискованных слайдов).
