...
# Заметки сборки о том, что текст не влез в родной кегль: ужат, усечён
# или не помещается даже кеглем подписи (`compose.builder._draw_slot`,
# `_fit_cloned_text`).
_CAPACITY_NOTE_MARKERS = ("кегл", "усечен", "усечён", "не помещается", "не влезает")
