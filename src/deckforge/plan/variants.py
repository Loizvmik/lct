...
_DIVIDER_LABELS = ("Контекст", "Данные", "Решение", "Результаты", "Риски", "План", "Итоги")
...
def _content_char_len(slide: SlideSpec) -> int:
    ...
    единица, что `Capacity.max_chars_per_item` (`template/patterns.py`).

    Известный долг брифа ("Планирование обязано учитывать, что шаблон
    умеет") оказывается верным и здесь по-новому: разброс `max_chars_per_
    item` у раскладок ОДНОГО `kind` в реальном шаблоне велик (на VK Tech —
    `cards` от 14 до 252 знаков на элемент, живой замер при подготовке этой
    задачи) — совпадения по ЧИСЛУ элементов (`_item_count`) одного недостаточно,
    раскладка с тем же числом карточек, но втрое меньшей текстовой
    вместимостью, даёт видимое переполнение текста на живом рендере (найдено
    обязательной проверкой этой задачи на `queue-latency`, см. отчёт).
    `_pick_pattern_id` ниже сверяется и с этим числом, не только с
    `_item_count`."""
    ...
    for block in slide.blocks:
        if isinstance(block, TextBlock):
            ...
            lengths.extend(len(item) for item in block.items)
...
def _item_count(slide: SlideSpec) -> int | None:
    """Фактический объём содержания слайда — та же логика, что `writer.
    _content_item_count`, самостоятельная копия (не импорт чужой приватной
    функции — тот же принцип независимости соседних модулей, что и везде в
    проекте, см. докстроку `template/patterns.py` про `_layouts`)."""
    for block in slide.blocks:
...
def _pattern_rank_key(
    ...
    return (char_bucket, cap_bucket, avoid_penalty, kind_rank, decor_bias, capacity_bias, roominess_bias, -p.score)
