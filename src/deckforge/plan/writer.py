...
def _flag_repeated_headlines(slides: list[SlideSpec]) -> None:
    """Помечает finding'ом пары слайдов, чьи заголовки, вероятно, несут один
    и тот же главный факт другими словами — код-бэкстоп поверх правила
    ...
    другими словами через него не проходит. Не переписывает и не
    объединяет слайды (для этого нужен ещё один вызов модели, вне бюджета
    времени колоды) — только честно помечает находкой, как и любое другое
    ...
    for i in range(len(slides)):
        for j in range(i + 1, len(slides)):
            ...
            if overlap >= _HEADLINE_OVERLAP_THRESHOLD:
